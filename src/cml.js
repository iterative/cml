...
class CML {
  ...
  async pr_create(opts = {}) {
    ...
    if (!skip_ci && source.includes('cmlpr')) {
      console.log(
        'Current branch is generated by CML. Skipping to avoid recursive execution.'
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        await exec(`git pull`);
        await exec(`git pull origin ${source}`);
...
re-run and update (force-push) this Pull Request with updated files.
`;

      try {
        console.log(
          await driver.pr_create({
            source: target,
            target: source,
            title,
            description
