...
describe('CML e2e', () => {
  ...
  test('cml-publish assets/logo.png', async () => {
    const output =
        await exec(`echo none | node ./bin/cml-publish.js assets/logo.png`);
  ...
  test('cml-publish assets/test.svg', async () => {
    const output =
        await exec(`echo none | node ./bin/cml-publish.js assets/test.svg`);
  ...
  test('cml-publish assets/logo.pdf to file', async () => {
    ...
    await exec(
        `echo none | node ./bin/cml-publish.js assets/logo.pdf --title 'this is awesome' --file ${
            file}`);
